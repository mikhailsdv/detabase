...
program
	...
	.action((database, {filename, ...options}) => {
		handleErrorAsync(async () => {
			...
			console.log("Database size:", data.count)
...
program
	...
	.action((database, options) => {
		handleErrorAsync(async () => {
			...
			const data = await detaApi.query({database, ...options})
			console.log(
				`There are ${data.count} items${options.query ? " matching the query" : ""}.`
			)
		...
		handleErrorAsync(async () => {
			...
			const results = await reduceAsync(
				...
				async (part, counter, acc) => {
					console.log(
						"Cloning...",
						`${counter * partLength + part.length}/${data.count}`
					)
					try {
			...
			console.log(`Succesfuly put ${data.count} items into "${newName}".`)
			console.log(`Query took ${timer()} seconds.`)
		})
	})
				...
				await reduceAsync(
					...
					async (item, counter, acc) => {
						try {
							...
							console.log(`Deleted items ${counter + 1}/${data.count}`)
						} catch (err) {
							displayError(err)
						}
					},
...
program
	...
	.action((database, options) => {
		handleErrorAsync(async () => {
			...
			const timer = benchmark()
			const data = await detaApi.query({database, ...options})
			data.count && options.json
				? console.log(jsonStringify(data.items))
				: printTable(data.items)
			console.log(`There are ${data.count} items matching the query`)
			console.log(`Query took ${timer()} seconds.`)
		})
	})
	...
	.action((database, key, options) => {
		handleErrorAsync(async () => {
			...
			if (key) {
				...
				console.log(`Found ${data.count} items. Starting to delete...`)
				...
				console.log(`Succesfuly deleted ${data.count} items from "${database}".`)
				console.log(`Process took ${timer()} seconds.`)
			} else {
				return console.error(
	...
	.action((database, key, options) => {
		handleErrorAsync(async () => {
			...
			if (key) {
				...
				console.log(`Found ${data.count} items. Starting to update...`)
				...
				await reduceAsync(
					...
					async (item, counter, acc) => {
						try {
							...
							console.log(`Updated items ${counter + 1}/${data.count}`)
						} catch (err) {
							displayError(err)
						}
					},
				...
				console.log(`Succesfuly updated ${data.count} items of "${database}".`)
				console.log(`Process took ${timer()} seconds.`)
			} else {
				return console.error(
					"One of the arguments is required. Please, set a key or query (-q <query>)."
